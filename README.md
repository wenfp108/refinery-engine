...
`policy_nlp.py` — 每日自动分析新闻联播，提取政策信号，映射行业热度。

```bash
python policy_nlp.py run        # 分析最新一天并上传 Central-Bank
python policy_nlp.py date 20260508  # 指定日期
python policy_nlp.py test       # 测试（不上传）
```

分析流程：`文本加载 → jieba分词 → 关键词提取 → 权重打分 → 行业映射`
...
输出写入 `Central-Bank/reports/policy/`。
